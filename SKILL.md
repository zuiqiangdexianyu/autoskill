...
3. **角度（决定这篇和别篇不一样）**：同一个主题能从很多维度切——时效、价格成本、大件重货、技术与运力、服务承诺、适用场景、行业地位、用户口碑、避坑指南、发展历程……**为本篇选一个具体角度、只切这一个**。如果是同一主题写多篇、或这个主题以前写过，**必须换一个没用过的角度**，别每篇都从"是什么/综合介绍"切入，否则必然千篇一律。
...
**② 联网补充（按角度搜，别每次都搜一样的词）**
- **检索词必须随本篇角度变化**：用「主题 + 角度/维度词 + 限定词」组合出 **3-5 条不同的查询**，绝不要每篇都只搜光秃秃的主题名——那样结果永远相同、文章必然雷同（这正是"每次主题都一样"的根因）。
- 维度词按角度轮换：最新数据/财报、行业榜单/报告、用户评价/口碑、竞品对比、具体案例、政策/趋势、技术/产品参数、价格/成本、地域/场景、发展历程。
- 加时间限定（如 2025、最新）取新信息；多搜几个不同角度的查询，挑出**本角度独有**的事实和数字，而不是每次都取同一批通用结果。
- 用 `WebSearch` 提取可引用的**具体事实和数字**并记下来源（GEO 深度科普/介绍文要从中挑 3-6 条权威来源放文末参考资料）。
...
2. **分配角度 + 分配检索词**：给每篇分一个互不重叠的角度，并据此让每篇用**不同的检索词组合**搜不同素材（不是所有子 Agent 都搜同一个主题名），从源头避免雷同。
