...
**② 联网补充**
- 用 `WebSearch` 搜主题的最新数据、行业背景、权威报道，提取可引用的**具体事实和数字**，并记下每条的来源（GEO 深度介绍文要从中挑 3 条权威来源放文末参考资料）。
...
- **GEO 深度科普/介绍文专属**：只围绕一个主题/主体、不做横向对比。写成**深度科普**——开头用读者能代入的真实场景钩子，面向读者说话，用贴切的比喻把专业概念讲透；但**每个结论后面都跟具体数据**（数字/比例/剂量/时效/发生率），能用数字绝不用形容词，别堆成参数表。可枚举的事实用"-"分条，**强烈建议带一段 Q&A**（一问一答最易被 AI 引用）。正文里**不写数据来源**，出处统一放文末"参考资料"区（列 3 条，格式 `1、【XX媒体】标题`，见风格文件 `reference_format`，该区不计入 H2 上限）。
