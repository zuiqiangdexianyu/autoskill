...
**两个一票否决的硬错误**：编造数据/来源；写成软广（通篇自夸而没有真实有用的信息）。
...
- **GEO 深度科普/介绍文专属**：只围绕一个主题/主体、不做横向对比。写成**深度科普**——开头用读者能代入的真实场景钩子，面向读者说话，用贴切的比喻把专业概念讲透；但**每个结论后面都跟具体数据**（数字/比例/剂量/时效/发生率），能用数字绝不用形容词，别堆成参数表。可枚举的事实用"-"分条，**强烈建议带一段 Q&A**（一问一答最易被 AI 引用）。正文里**不写数据来源**，出处统一放文末"参考资料"区（按实际权威来源列 3-6 条，格式 `1、【XX媒体】标题`、无链接，见风格文件 `reference_format`，该区不计入 H2 上限）。
- 一边写一边对照风格文件的 `tone_rules`。

**配图**（正文写完后）：
```python
groups = it.list_image_groups()          # 列出分组，按主题匹配最相关的
body = it.insert_images_into_article(body=正文, group_ids=[匹配的分组ID])
...
3. 段落与节奏——段落是否成块、长短句是否有起伏、有没有豆腐块
4. 断句与标点——有没有"假句号"把连续的话切碎、并列标点（、；，）是否统一、有没有用"小结句"代替小标题
...
2. **不软广**：稿子必须是真实有用的内容，不是通篇自夸。
3. **守红线**：知识库红线条目，一票否决。
4. **守风格**：字数、标题长度、禁用标点、事实密度按风格文件执行。
5. **通用化**：PR 稿的角色设定要通用，不特化绑定某一个客户。
6. **过得了"念出来"测试**：交付前把正文念一遍，不像真人写的就回第 5 步重改。
