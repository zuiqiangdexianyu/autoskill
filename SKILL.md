...
4. **风格**：用户指定了就用指定的；没指定，时效/参数/对比类主题默认 GEO，观点/体验/避坑类主题默认自媒体——并简短告诉用户你选了哪个。
...
**② 联网补充**
- 用 `WebSearch` 搜主题的最新数据、行业背景、竞品动态，提取可引用的**具体事实和数字**（带来源）。
...
3. **写小标题**：自媒体的小标题要有钩子；GEO 的小标题要写成"用户真实会搜的问题"。H2 数量不超过风格上限。
...
- **段落要成块**：每个 H2 下 2–4 段，每段 4–8 句。同一个意思的几句话用逗号/分号连成一段，**不要一句一段拆成豆腐块**。
...
3. 节奏与分节——段落是否成块、长短句是否有起伏、有没有豆腐块、有没有用"小结句"代替小标题
4. 态度——有没有作者的判断，是不是全程中性陈述
5. 结尾——是否具体收住，有没有强行升华
6. 真人味——念出来像不像真人写的
